...
public class HubstafftalentScript {
	private BasePo basePo;
	private HubstafftalentPo hubstafftalentPo;

	private final String techData = "hubsstafftalent";
	private final String portalName = "HubstaffTalent";
	...
	public void startup() {
		...
		hubstafftalentPo = new HubstafftalentPo(page);
	}
	...
	public void Frontend() throws IOException {
		...
		hubstafftalentPo.clickBrowseJobs();
		hubstafftalentPo.clickJobType();
		...
		for (int i = 0; i < technologiesData.size(); i++) {
			...
			hubstafftalentPo.enterTechnology(technologiesData.get(i));
			if (!hubstafftalentPo.checkNoMatchJobsFoundIsVisible()) {
				count = hubstafftalentPo.selectJobs(screenshotDir, count);
				screenshotCount = hubstafftalentPo.getScreenshotCount();
			} else {
				count = 0;
				screenshotCount = 0;
